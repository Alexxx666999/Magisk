...
use base::{Chunker, LoggedResult, ResultExt, WriteExt};
use bytemuck::bytes_of_mut;
use bzip2::{Compression as BzCompression, write::BzDecoder, write::BzEncoder};
use flate2::{Compression as GzCompression, write::GzDecoder, write::GzEncoder};
use lz4::{
...
finish_impl!(GzEncoder<W>, GzDecoder<W>, BzEncoder<W>, XzEncoder<W>);
...
impl FakeReader {
    ...
    unsafe fn set_data(&self, data: &[u8]) {
        ...
        // no way to specify this in Rust's lifetime marker system, so we transmute all
...
pub fn get_decoder<'a, W: Write + 'a>(format: FileFormat, w: W) -> Box<dyn WriteFinish<W> + 'a> {
    match format {
        ...
        FileFormat::ZOPFLI | FileFormat::GZIP => Box::new(GzDecoder::new(w)),
        _ => unreachable!(),
    }
}
...
pub fn compress<R: Read, W: Write>(
    ...
    let mut encoder = get_encoder(format, write);
    std::io::copy(&mut read, encoder.as_mut()).log_ok();
...
pub fn decompress<R: Read, W: Write>(
    ...
    let mut decoder = get_decoder(format, write);
    std::io::copy(&mut read, decoder.as_mut()).log_ok();
...
pub fn compress_fd(format: FileFormat, in_fd: RawFd, out_fd: RawFd) {
    ...
    compress(format, in_file.deref_mut(), out_file.deref_mut()).log_ok();
}
...
pub fn decompress_bytes_fd(format: FileFormat, in_bytes: &[u8], in_fd: RawFd, out_fd: RawFd) {
    ...
    let _: LoggedResult<()> = try {
        let mut encoder = get_decoder(format, out_file.deref_mut());
...
pub fn compress_bytes(format: FileFormat, in_bytes: &[u8], out_fd: RawFd) {
    let mut out_file = unsafe { ManuallyDrop::new(File::from_raw_fd(out_fd)) };
    compress(format, Cursor::new(in_bytes), out_file.deref_mut()).log_ok();
}

pub fn decompress_bytes(format: FileFormat, in_bytes: &[u8], out_fd: RawFd) {
    let mut out_file = unsafe { ManuallyDrop::new(File::from_raw_fd(out_fd)) };
    decompress(format, Cursor::new(in_bytes), out_file.deref_mut()).log_ok();
}
